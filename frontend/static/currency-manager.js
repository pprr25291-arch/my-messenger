class CurrencyManager {
    ...
    async init() {
        ...
        console.log('✅ CurrencyManager initialized, user:', this.currentUser);
    }

    // Метод для безопасного получения имени пользователя
    getCurrentUser() {
        const usernameElement = document.getElementById('username');
        if (usernameElement) {
            return usernameElement.textContent || usernameElement.value || 'anonymous';
        }
    ...
    saveLocalData() {
        try {
            ...
            console.log('💾 Currency data saved locally for:', this.currentUser);
        } catch (error) {
...
class GiftManager {
    ...
    getReceivedGifts(username) {
        const userGifts = this.getUserGifts(username);
        return userGifts.filter(gift => gift.from && gift.from !== 'purchase');
    }
    ...
    async buyAndSendGift(gift, receiver) {
        ...
        try {
            ...
            await this.sendGift(sender, receiver, gift.id);
    ...
    getRandomGift(rarity = 'common') {
        ...
        return giftsByRarity[randomIndex];
    }
