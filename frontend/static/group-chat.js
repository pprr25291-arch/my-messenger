...
removeDuplicateGroups(groups) {
    ...
    return groups.filter(group => {
        ...
        if (!groupId || seen.has(groupId)) {
            console.log(`⚠️ Removing duplicate group: ${groupId} - ${group.name}`);
            return false;
        }
        seen.add(groupId);
        return true;
...
async loadUserGroups() {
    try {
        ...
        // Пробуем каждый endpoint последовательно
        ...
        // Обработка и форматирование групп
        const processedGroups = this.processGroups(groups, currentUser);
        console.log(`✅ Final processed groups: ${processedGroups.length}`);
        
        return processedGroups;
        
    } catch (error) {
        console.error('❌ Error in GroupChatManager.loadUserGroups():', error);
        return [];
    }
}
processGroups(groups, currentUser) {
    if (!Array.isArray(groups)) {
        console.log('⚠️ groups is not an array:', groups);
        return [];
    }
    
    const processedGroups = groups.map(group => {
        try {
            const groupId = group.id || group._id || group.groupId;
            if (!groupId) {
            ...
            if (!isMember && !group.isGlobal) {
                console.log(`⏩ User ${currentUser} is not a member of group ${group.name}, skipping`);
                return null;
            }
            
            // Базовое форматирование группы
            ...
            if (group.lastMessage) {
                processedGroup.lastMessage = {
                    text: group.lastMessage.message || group.lastMessage.text || '',
                    timestamp: group.lastMessage.timestamp || group.lastMessage.date,
                    sender: group.lastMessage.sender,
                    type: group.lastMessage.messageType || group.lastMessage.type || 'text',
                    isOwn: group.lastMessage.sender === currentUser
                };
            }
    ...
    const validGroups = processedGroups.filter(group => group !== null);
    const uniqueGroups = this.removeDuplicateGroups(validGroups);
    
    // Сортируем: сначала глобальные группы, потом остальные
...
async checkAndAddToDefaultGroups() {
    try {
        ...
        // Проверяем, есть ли у пользователя группы
...
async getGroupInfo(groupId) {
    try {
        // Проверяем, есть ли группа в локальном кэше
        if (this.currentGroup && this.currentGroup.id === groupId) {
            return this.currentGroup;
        }
        
        // Проверяем в общем списке групп
        const localGroup = groups.find(g => g.id === groupId);
        if (localGroup) {
            return localGroup;
        }
        
        // Пробуем загрузить с сервера только если у нас есть endpoint
        try {
            ...
            console.log('⚠️ Group info fetch failed, using fallback:', fetchError.message);
...
async loadGroupMessages(groupId) {
    try {
        ...
        try {
            ...
            // ДОБАВЬТЕ проверку на ошибки HTTP
            if (!response.ok) {
        ...
        if (container) {
            container.innerHTML = `
                <div class="no-messages">
                    <div>📝 Начните общение в группе!</div>
...
closeAllModals() {
 ...
 useTestUsers() {
  ...
  async createGroup() {
    ...
    try {
        ...
        const filteredUsers = Array.from(this.selectedUsers).filter(username => username !== currentUser);
        const allMembers = [currentUser, ...filteredUsers];
 ...
 async openGroupChat(group) {
    ...
    // Загружаем информацию о группе с обработкой ошибок
    let memberCount = 0;
    let groupName = group.name || 'Группа';
    
    try {
        const groupInfo = await this.getGroupInfo(group.id);
        memberCount = groupInfo?.members?.length || group.members?.length || 0;
        groupName = groupInfo?.name || group.name || 'Группа';
        
        // Обновляем текущую группу с полученной информацией
        this.currentGroup = { ...this.currentGroup, ...groupInfo };
    } catch (error) {
        console.error('❌ Error loading group info, using defaults:', error);
