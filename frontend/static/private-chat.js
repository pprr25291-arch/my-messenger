                    ...
                    <div class="chat-top-bar">
                        ...
                        <div class="chat-user-info" style="${isMobile ? 'flex: 1;' : ''}">
                            <span class="user-avatar">
                                <img src="/static/default-avatar.png" class="user-avatar-img" alt="" style="width: 40px; height: 40px; border-radius: 50%;">
                            </span>
...
async updateUserAvatar(username) {
   ...
   getDefaultAvatarUrl() {
    // Создаем SVG аватар по умолчанию вместо ссылки на файл
    const svg = `
        <svg xmlns="http://www.w3.org/2000/svg" width="200" height="200" viewBox="0 0 200 200">
            <rect width="200" height="200" fill="#007bff" rx="100" ry="100"/>
            <text x="100" y="110" text-anchor="middle" fill="white" font-size="80" font-family="Arial, sans-serif">👤</text>
        </svg>
    `;
...
async loadUserAvatarSafe(username, maxRetries = 2) {
    ...
    // Проверяем кэш, но только если это не дефолтный аватар
    if (this.avatarCache.has(username)) {
        const cached = this.avatarCache.get(username);
        if (cached && cached !== this.getDefaultAvatarUrl()) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            // Проверяем, что URL валидный и не дефолтный
...
handleAvatarError(img) {
    ...
    // Сохраняем исходный alt для логирования
    const originalAlt = img.alt || img.dataset.username || '';
    
    // Проверяем, не является ли это группой
    const isGroup = originalAlt.includes('👥') || originalAlt.includes('группа') || 
                   img.closest('.group-item') || 
                   this.isGroupName(originalAlt);
    ...
    // Удаляем все обработчики ошибок
    img.onerror = null;
    img.onload = null;
    
    // Обновляем кэш
    if (originalAlt) {
        this.avatarCache.set(originalAlt, img.src);
    }
}
async preloadConversationAvatars(conversations) {
    // Предзагружаем аватары для первых 5 диалогов
    const conversationsToPreload = conversations.slice(0, 5);
    
    for (const conversation of conversationsToPreload) {
        if (!conversation.isGroup && conversation.username) {
            try {
                await this.loadUserAvatarSafe(conversation.username);
            } catch (error) {
                // Игнорируем ошибки прелоада
                console.log(`⚠️ Preload failed for ${conversation.username}:`, error.message);
            }
        }
    }
}
  async loadUserGroupsForPrivateChat() {
    try {
...
async displayConversations() {
    ...
    try {
        ...
        // Прелоад аватаров
        this.preloadConversationAvatars(conversationsWithAvatars);
...
function setupChatNavigation() {
    // Базовая навигация для чата
    console.log('Setting up chat navigation...');
}
...
function initSocket() {
    ...
    if (!window.socket) {
        ...
        // Здесь должна быть логика инициализации socket
    }
}
